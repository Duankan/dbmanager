...
export default {
  ...
  props: {
    type: {
      type: String,
      validator(value) {
        return ['attribute', 'space', 'composite'].includes(value);
      },
      default: 'composite',
    },
    title: {
  ...
  methods: {
    ...
    handleSubmit(name) {
      ...
      this.$store.commit(types.REMOVE_BUS_FIELD, this.$store.state);
      this.$store.commit(types.REMOVE_BUS_ATTRIBUTE, this.$store.state);
